            ...
            ordered.append(
                {
                    ...
                    "word_review_start": line.get("start"),
                    "word_review_end": line.get("end"),
...
def assign_instrumental_run(
    ...
    if previous_line and following_line:
        ...
        previous_natural_end = as_float(
            previous_line.get("natural_end"),
            previous_last_word_start + max(next_line_gap_seconds, 0.25),
        )

        # Instrumentals are display gaps, so use the surrounding lyric line
        # boundaries rather than guessing from only word starts.
        # This usually places the placeholder after the previous sung line has
        # finished and keeps it clear of the next lyric line starting.
        gap_start = max(previous_start + 0.1, previous_natural_end + next_line_gap_seconds)
        gap_end = following_start - next_line_gap_seconds

        if gap_end - gap_start < total_minimum:
            fallback_duration = max(total_minimum, instrumental_fallback_seconds * count)
            gap_start = max(previous_start + 0.1, following_start - next_line_gap_seconds - fallback_duration)
...
def build_draft(
    ...
    for line in ordered_lines:
        ...
        natural_end = as_float(
            line.get("word_review_end"),
            as_float(last_word.get("end"), as_float(last_word.get("start")) + 0.5),
        )
        natural_end = max(raw_start + 0.1, natural_end)
        ...
        line["natural_end"] = round_time(natural_end)
        line["confidence"] = "draft"
        line["locked"] = False
        line["anchor"] = False
    ...
    return {
        ...
        "editor_notes": [
            ...
            "Instrumental placeholders use the previous lyric line end and next lyric line start where possible.",
